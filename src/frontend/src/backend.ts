...
export class Backend implements backendInterface {
    ...
    async deleteNakshatra(arg0: string): Promise<boolean> {
        if (this.processError) {
            try {
                const result = await this.actor.deleteNakshatra(arg0);
                return result;
            } catch (e) {
                this.processError(e);
                throw new Error("unreachable");
    ...
    async readNakshatra(arg0: string): Promise<Nakshatra | null> {
        if (this.processError) {
            try {
